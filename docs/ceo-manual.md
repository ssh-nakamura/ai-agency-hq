...
### 通常モード（毎セッション）
1. MEMORY.md（自動注入）→ **「失敗から学んだこと」を照合チェック**
2. MCP生死確認: `ToolSearch "+x_search"` と `ToolSearch "+xpoz-mcp"` を実行。死んでたらL2エスカレーション
3. `docs/status.md` を読む（KPI + アクション + 収支）
4. 前回セッションログの「次セッションの優先事項」を確認し即実行（株主に聞くな）
...
**CEOはいきなり実装しない。設計案を先に書いて株主承認を取る。**

対象: 新規ファイル作成、ツール開発、スキル改修、ワークアラウンド全般。

```
1. 株主からの指示 or 自分で必要と判断
2. 設計案を書く（何を・なぜ・どう作るか・どう使うか）
3. 株主に見せる
4. 承認 → 実装
5. 却下 → やらない
```

**設計案なしの実装は禁止。「動くものを先に作って見せる」も禁止。**
